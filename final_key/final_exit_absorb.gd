...
# false：默认未激活，取得钥匙后再由 FinalKey 激活。
@export var starts_active: bool = false

# 指向锅上的漩涡特效根节点。
@export var vortex_root: Node3D

# 找不到 FinalKey 控制器时，是否直接调用公共 UI。
@export var auto_show_win: bool = false

var tools_root: Node3D
var _active: bool = false
var _cutaway_visible: bool = true
...
func _ready() -> void:
	...
	# 兼容旧场景：AbsorbSfx 也可能是当前 Area3D 的同级节点。
	if absorb_sfx == null:
		absorb_sfx = (
			get_node_or_null("../AbsorbSfx")
			as AudioStreamPlayer
	...
	if absorb_sfx == null:
		push_error(
			"final_exit_absorb.gd：没有找到 AbsorbSfx"
		)
	elif absorb_sfx.stream == null:
		push_error(
			"final_exit_absorb.gd：AbsorbSfx 没有设置音频文件"
		)
...
func setup(root: Node3D) -> void:
	...
	var unlocked := false
	if tools_root != null and tools_root.has_method("can_win"):
		unlocked = bool(tools_root.call("can_win"))
...
func set_active_look(value: bool) -> void:
	if vortex_root == null:
		return

	vortex_root.visible = value
	_set_particles_emitting_recursive(
		vortex_root,
		value
	)


func apply_cutaway_visibility(
	wall_visible: bool
) -> void:
	_cutaway_visible = wall_visible
...
func _refresh_monitoring() -> void:
	var should_monitor := (
		_active
		and _cutaway_visible
		and not is_absorbing
	)
...
func _set_particles_emitting_recursive(
	node: Node,
	emitting: bool
) -> void:
...
func _begin_absorption(player: Node3D) -> void:
	...
	# 吸入结束后才通知 FinalKey，再由它确认钥匙状态并显示胜利。
	if (
		tools_root != null
		and tools_root.has_method("trigger_win")
	):
		tools_root.call(
			"trigger_win",
			player
