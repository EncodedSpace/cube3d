	...
	for child in node.get_children():
		if child is Node3D:
			...
			if (
				...
				and ("StaticBox" in n or "MovableBox" in n or "DeadlyBox" in n or n == "Tool_Z" or n.begins_with("Tool_Z"))
				and "EXIT" not in n
			):
...
func _bind_props_to_walls() -> void:
	...
	for pattern in ["*StaticBox*", "*MovableBox*", "*DeadlyBox*", "*Portal*", "*F_Trigger*", "*B_Tool*", "*D_Wall*", "*G_Tool*", "*Tool_X*", "*Tool_Y*", "*Tool_Z*"]:
	...
	for prop in props:
		...
		# Falling rigid bodies must not be controlled by camera cutaway.
		# Otherwise a box moved onto the hidden top face is made invisible,
		# its collision is disabled, and it remains frozen instead of falling.
		if prop is RigidBody3D and _is_fallable_body(prop as RigidBody3D):
			prop.visible = true
			continue
...
func _animate_flip(rot: Quaternion, player: Node3D, _keep_relative_facing: bool = false) -> void:
	...
	# Release the rotating cube, refresh wall cutaway, then explicitly restore
	# every falling rigid body. Falling bodies are intentionally excluded from
	# wall cutaway, so they need their own unfreeze step here.
	_hold_props_frozen = false
	update_cutaway_visibility()
	_release_fallable_props()

	player.set_physics_process(true)

	# Wait one physics frame so PhysicsServer receives the final transforms,
	# then wake the bodies again before checking whether they have settled.
...
func _release_fallable_props() -> void:
	for rb in _gather_fallable_bodies():
		...
		# Restore every collision shape, including nested shapes.
		for node in rb.find_children("*", "CollisionShape3D", true, false):
