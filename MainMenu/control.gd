...
func _refresh_buttons() -> void:
	...
	btn1.disabled = false
	# 后续关卡按顺序解锁
	btn2.disabled = not LevelProgress.is_completed("teach")
	btn3.disabled = not LevelProgress.is_completed("level1")
	btn4.disabled = not LevelProgress.is_completed("level2")
	btn5.disabled = not LevelProgress.is_completed("level3")
